...
const HIGH_CONFIDENCE_BLOCKED_PATTERN = /\b(?:porn(?:ography)?|xxx|nudes?|onlyfans|rape|kill\s+yourself|kys)\b/i;
...
export function validateCommunityText(value, { required = false, maxLength = 500 } = {}) {
  ...
  if (HIGH_CONFIDENCE_BLOCKED_PATTERN.test(text)) return { error: "Please remove explicit or threatening language." };
